...
版本号规则：`versionCode = 主*10000 + 次*100 + 修订`（2.2.2 → 20202）；**小改升修订号、大改升次版本号**。
...
| 09-19 | 2.2.2 | **铃声迟到 13 分 48 秒**（09:00 到点，09:13:48 才响） | 看门狗用协程 `delay()` 实现，**CPU 睡眠时不唤醒**；到点起播后设备重新入睡，定时器被冻到下次唤醒才补跑 | 看门狗改 **AlarmManager** `setExactAndAllowWhileIdle` 主通道（Doze 下唤醒 CPU），协程 delay 降为副通道双保险 |
| 09-19 | 2.2.2 | 同一秒「两次选片、两次起播」声音互叠 | `forceRestartSession` 只 `sessionSeq++`，但选片链路横跨读设置/两级同步等挂起点，旧协程醒来后照旧起播 | `selectJob` 取消旧链路（拦挂起点）+ `startMainWith` 前场次比对（拦已过挂起点），双保险 |
...
4. **息屏链路上，任何依赖 CPU 常醒的定时都不可信** —— `delay()` / `Handler.postDelayed()`
   在 Doze 下会被连进程一起冻住，要到下次系统唤醒才补跑（实测拖了 13 分钟）。
   凡是「必须在某个时刻发生」的动作，一律走 **AlarmManager**（闹钟类用 `setAlarmClock`，
   对用户不可见的补救类用 `setExactAndAllowWhileIdle`，避免状态栏挂闹钟图标）。
   协程 `delay` 只能当「设备清醒时更早触发」的优化，绝不能当唯一保障。
   > 排查提示：若日志里**多个无关的定时任务在同一时刻集中执行**，基本可断定是设备从休眠中
   > 一起醒来补跑积压定时器，而非任务本身慢 —— 这是判断「CPU 睡眠冻住定时」的关键特征。
5. **状态位（如 `sessionSeq`）让位 ≠ 异步链路让位**。任何横跨挂起点的旧链路，
   都必须同时做两件事：取消旧协程 + 在关键动作前重新比对状态位。
   只做其中一件都会留下「旧链路继续写状态」的窗口。
