...
#
# 用法：
#   1) 把 dyproxy.cjs、.env、frpc.local.toml 放到 /opt/dyproxy/
#   2) sudo bash setup-linux-vm.sh
#
# 脚本会依次：装依赖 → 装 Node → 装 frpc → 建运行用户 → 检查配置 →
#             写 systemd 服务 → 启动 → 自检
#
...
if command -v node >/dev/null 2>&1 && [ "$(node -v)" = "$NODE_VER" ]; then
  ...
  OK=0
  # 优先官方源，失败则退回国内镜像（npmmirror，阿里维护）
...
MISSING=0
for f in dyproxy.cjs .env frpc.local.toml; do
  if [ -f "$APP_DIR/$f" ]; then
    printf '  ✓ %s\n' "$f"
  else
    printf '  ✗ 缺少 %s\n' "$APP_DIR/$f" >&2
...
chown "$RUN_USER:$RUN_USER" "$APP_DIR/dyproxy.cjs" "$APP_DIR/.env" "$APP_DIR/frpc.local.toml"
chmod 600 "$APP_DIR/.env" "$APP_DIR/frpc.local.toml"
echo "  权限已收紧（.env / frpc.local.toml 仅属主可读）"

# ---------- 5. systemd 服务 ----------
log "5/6 写入 systemd 服务"
...
echo
echo "--- frpc 最近日志 ---"
journalctl -u frpc -n 10 --no-pager 2>/dev/null || true
