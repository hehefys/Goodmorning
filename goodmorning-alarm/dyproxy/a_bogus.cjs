// a_bogus.cjs — 抖音 web 端 a_bogus 签名生成器（单文件，零依赖）
//
// 算法来源：https://github.com/ylcangel/douyin_sign  (Apache-2.0, 作者 AngelToms)
// 对应抖音版本：1.0.1.19-fix.01
// 本文件 = sm3.js + utils.js 合并，仅追加 Node 端浏览器环境桩与导出入口。
//
// 用法：
//   const { abogus } = require('./a_bogus.cjs');
//   const sig = abogus('device_platform=webapp&aid=6383&...', { ua: '...', screenW: 1707, screenH: 960 });
//
// 注意：算法内部含 Math.random()，每次输出不同，无法本地自验，只能由线上接口判定有效性。
// 注意：本文件依赖非严格模式下的隐式全局变量，请勿添加 'use strict'。
// 注意：screen.sizeWidth/sizeHeight 必须与 screenW/screenH 一致，否则输出长度会从 188 变成 176。

var G_DEBUG = 'debug';
var G_RELEASE = 'release';
var programVersion = 'release';
var EnvTestTurnOn = false;
var PerformanceTestTurnOn = false;
...
const ENV = {
  ...
  platform: 'Win32',
  screenW: 1707, screenH: 960,
  availW: 1707, availH: 960,
  innerW: 1707, innerH: 960,
  outerW: 1707, outerH: 960,
};

const navigator = { userAgent: ENV.ua, platform: ENV.platform, vendorSubs: {} };
const screen = { width: ENV.screenW, height: ENV.screenH, availWidth: ENV.availW, availHeight: ENV.availH,
                 sizeWidth: ENV.screenW, sizeHeight: ENV.screenH };
const window = { innerWidth: ENV.innerW, innerHeight: ENV.innerH, outerWidth: ENV.outerW, outerHeight: ENV.outerH,
                 screen: null, onwheelx: undefined };
const document = {};
...
// ---- 对外入口 ----
programVersion = G_RELEASE;

function syncEnv() {
  navigator.userAgent = ENV.ua;
  navigator.platform = ENV.platform;
  screen.width = ENV.screenW;  screen.height = ENV.screenH;
  screen.availWidth = ENV.availW; screen.availHeight = ENV.availH;
  screen.sizeWidth = ENV.screenW; screen.sizeHeight = ENV.screenH;
  window.innerWidth = ENV.innerW; window.innerHeight = ENV.innerH;
  window.outerWidth = ENV.outerW; window.outerHeight = ENV.outerH;
  window.screen = screen;
}

// query: 不含前导 '?' 的完整 query 串
// opts : {ua, platform, screenW, screenH, availW, availH, innerW, innerH, outerW, outerH}
function abogus(query, opts) {
  if (opts) Object.assign(ENV, opts);
  syncEnv();
  enterPageTs = +Date.now();
  return makeABogus(query, 0);
}

module.exports = { abogus, ENV };
