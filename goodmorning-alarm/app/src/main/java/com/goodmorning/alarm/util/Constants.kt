...
object Constants {
    ...
    /**
     * 起播看门狗：响铃卫兵置位后，若该时长内**既无主音频也无副音频**，强制走兜底铃声。
     *
     ...
     * 本看门狗是该场景的最后保险。
     *
     * ⚠️ 判据必须是「完全没声音」：副音频衬托期本来就不播主音频，
     * 只看主音频会误判（2026-09-17 实测：衬托 5s 与看门狗 5s 撞点）。
     * 时长取 8s：覆盖冷启动时「副音频已启动 → 真正出声」的初始化窗口（实测约 5s）。
     */
    const val RING_WATCHDOG_MS = 8_000L
